...
@testset "OrderedDict of reals" begin
    ...
    val1, grad1 =
        NonconvexCore.value_gradient(NonconvexCore.getineqconstraint(vec_model, 1), pvec)
    @test val1 == g(un(pvec), 2, 0)
    @test grad1 == Zygote.gradient(pvec -> g(un(pvec), 2, 0), pvec)[1]

    val2, grad2 =
        NonconvexCore.value_gradient(NonconvexCore.getineqconstraint(vec_model, 2), pvec)
    @test val2 == g(un(pvec), -1, 1)
    @test grad2 == Zygote.gradient(pvec -> g(un(pvec), -1, 1), pvec)[1]

    vals, jac =
        NonconvexCore.value_jacobian(NonconvexCore.getineqconstraints(vec_model), pvec)
    @test [val1, val2] == vals
    @test [grad1 grad2]' == jac

    vals, jac =
        NonconvexCore.value_jacobian(NonconvexCore.getobjectiveconstraints(vec_model), pvec)
